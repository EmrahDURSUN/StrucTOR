...
namespace StructureCreator
{
    ...
    /// </summary>
    class RemoveArrowCapsule : CommandCapsule
    {
        public const string CommandName = "ConstructorAddIn.C#.V19.RemoveArrowCapsule";

        public RemoveArrowCapsule()
            ...
            try
            {
                ...
                foreach (DesignBody b in Window.ActiveWindow.Document.MainPart.Bodies)
                {
                    foreach(String s in arrows)
                    {
                        if (!s.Equals(""))
                        {
                            if (b.Name.Equals("ExternalForce" + s))
                            {
                                b.Delete();
                            }
                        }
